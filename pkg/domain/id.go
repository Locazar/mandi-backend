...
const (
	...
	// PrefixGuest identifies the synthetic, no-login user id minted by
	// POST /auth/guest for anonymous browse (customer-web-app and similar).
	// It belongs to no row in `users` — see GuestLogin in auth.go — but it
...
var allPrefixes = []IDPrefix{
	...
	PrefixMobileUser, PrefixOtpSession, PrefixGuest,
